...
<div id="main">
    <form action="/Join/Signup_Join.jsp" name="form" method="post" onsubmit="return check_join()" onreset="">
        ...
        <p align="center"><img src="../img/Join.png" style="width: 1500px;"/></p>
        <hr class="hr"></hr>

        <fieldset><p align="center">학번: <input type="text" placeholder="학번 입력후 확인 해주세요!" minlength="9" maxlength="9"
                                               onKeyUp="check_Num(this, '');" onChange="check_Num(this, '');"
                                               class="text-form" name="std_id" value="" required> <input type="button"
                                                                                                         value="학번확인"
                                                                                                         class="btn btn-info"
                                                                                                         onclick="studentid_checek(this.form)">
        </p></fieldset>
        <!--name="" 값만 DB에서 읽을수 있도록 설정 -->

        <p align="center">아이디: <input type="text" size="50" value="" placeholder="아이디 입력후 중복확인 해주세요!" name="id"
                                      class="text-form" onkeydown="inputid_check()" required> <input type="button"
                                                                                                     class="btn btn-info"
                                                                                                     value="중복확인"
                                                                                                     onclick="check_id(this.form)">
            <input type="hidden" name="idcheck" value="idUncheck"></p>
        <p align="center">비밀번호: <input type="password" size="50" value="" minlength="4"
                                       placeholder="사용할 비밀번호를 정확히 입력하시오." name="pwd" class="text-form"
                                       onkeyup="check_pwd(this)" onchange="check_pwd(this)" required></p>
        <p align="center">학교이메일: <input type="text" name="email" class="text-form" required>@<select name="email2">
            <option value="bible.ac.kr" selected>bible.ac.kr</option>
            <option value="stu.bible.ac.kr">stu.bible.ac.kr</option>
        </select> <input type="button" name="email_check" class="btn btn-primary" value="인증하기"
                         onclick="email_c(this.form)">
            <input type="hidden" name="emailcheck" value="emUncheck"></p>
        <p align="center">휴대전화번호: <input type="tel" name="phone_num" class="text-form" onchange="check_tel(this.form)"
                                         maxlength="11" required></p>
        <p align="center"><input type="button" class="btn btn-success" value="Home"
                                 onClick="location.href='../index.jsp'"> <input type="reset" value="다시 쓰기"
                                                                                class="btn btn-success"> <input
                type="submit" value="회원가입" class="btn btn-success"></p>
